...
engine. Three boot-time interlocks now make that impossible. All three run
...
- **`--no-mic` requires loopback outbound targets.** Both the effective OSC
  ...
  target is a hard refusal naming the flag to pass, and you may instead point
  `companion.osc.host` / `companion.engine.host` at an explicit loopback
  address in `config.yaml`.
...
`/signal_snapshot` separates **registration** from **production**, because the
two used to be conflated. The row set comes from the registry's static `live`
descriptor flag — that only proves a key *exists*. What proves a key is being
produced is `writes` / `lastWriteHop`, counted in the real ParamCenter write
path, against the top-level `analyzerHops` clock:
...
- **The ENGINE is the sole writer of `states/<scene>/audio_state.yaml`.** The
  ...
  **parked and replayed** to the engine on reconnect. A replay the engine
  rejects snaps the value back and flashes the exact keys that reverted; until
  the engine has ruled, a pending group is locally authoritative and the
  reconnect reconciliation will not overwrite it.
