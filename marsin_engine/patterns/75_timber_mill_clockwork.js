/*
  timber_mill_clockwork
  Pattern matching technical and artistic specifications.
*/

export var localSpeed = 0.5;
export var gearSpeed = 0.5;
export var tickSharpness = 0.45;
export var boilerHeat = 0.35;
...
export function sliderGearSpeed(v) { gearSpeed = v; }
export function sliderTickSharpness(v) { tickSharpness = v; }
...
export function render3D(index, x, y, z) {
  var step = floor(index / 18) % 2;
  var tick = wave(tPhase * gearSpeed * 10.0);
  var isBar = ((viewMask & 1) != 0);
  var isVintage = ((viewMask & 128) != 0);
  var r = 0.0, g = 0.0, b = 0.0, w = 0.0, a = 0.0;
  if (isBar) {
    var active = (index + step * 9) % 18 < 6 ? 1.0 : 0.0;
    r = pr1 * active;
    g = pg1 * active;
    b = pb1 * active;
  }
  if (isVintage) {
    a = boilerHeat * tick;
    w = tickSharpness * (random(1) < 0.05 ? 1.0 : 0.0);
  }
  var u = uvIntensity;
