/*
  ...
  Three independent clouds wander in normalized x/y/z space. Their centres,
  ...
  Fixture-capability staging stays portable:
    ...
    FIX_VINTAGE_6  — warm golden droplets with matched W+A emitters.
    FIX_PAR        — restrained low-band stirring pools.
    FIX_TE_SIGN    — stable Identity field with one slow, readable bloom.
...
export function beforeRender(delta) {
  ...
  var speed = pow(2.0, (localSpeed - 0.5) * 4.0);
  var stir = 0.55 + flow * 0.90;

  aX = wrapPhase(aX + dt * 0.031 * speed * stir);
  aY = wrapPhase(aY + dt * 0.019 * speed * stir);
  aZ = wrapPhase(aZ + dt * 0.013 * speed * stir);
  aLife = wrapPhase(aLife + dt * 0.071 * speed);

  bX = wrapPhase(bX + dt * 0.023 * speed * stir);
  bY = wrapPhase(bY + dt * 0.037 * speed * stir);
  bZ = wrapPhase(bZ + dt * 0.017 * speed * stir);
  bLife = wrapPhase(bLife + dt * 0.053 * speed);

  cX = wrapPhase(cX + dt * 0.041 * speed * stir);
  cY = wrapPhase(cY + dt * 0.011 * speed * stir);
  cZ = wrapPhase(cZ + dt * 0.029 * speed * stir);
  cLife = wrapPhase(cLife + dt * 0.043 * speed);

  waterT = wrapPhase(waterT + dt * 0.021 * speed);
  vintageT = wrapPhase(vintageT + dt * 0.37 * speed);
  parT = wrapPhase(parT + dt * 0.047 * (0.45 + flow));
  signT = wrapPhase(signT + dt * 0.90 * speed * stir);
...
export function render3D(index, x, y, z) {
  ...
  var cloud = ca + cb * 0.92 + cc * 0.86;
  var density = clamp01(cloud * ink * 3.50);
  ...
  var blend = colorConc * colorConc * (3.0 - 2.0 * colorConc);
  ...
  // TE signs remain legible beneath three analytic XYZ dye clouds. Their
  // centres wander, bloom, and soften continuously; pixel order never enters
  // the topology, so the letters behave as one physical window into the ink.
  if (fixtureType == FIX_TE_SIGN) {
    var signBloom = wave(signT * 0.50 + 0.13);
    var signRadius = 0.18 + diffuse * 0.28 + signBloom * 0.05;
    var signRadiusSq = signRadius * signRadius;
    var signEdge = 2.30 - diffuse * 1.20 - signBloom * 0.35;

    var signACx = 0.28 + 0.22 * wave(signT + 0.07);
    var signACy = 0.50 + 0.18 * wave(signT * 0.75 + 0.21);
    var signACz = 0.52 + 0.22 * wave(signT * 0.50 + 0.43);
    var signBCx = 0.68 - 0.18 * wave(signT * 0.75 + 0.29);
    var signBCy = 0.58 + 0.20 * wave(signT * 0.50 + 0.61);
    var signBCz = 0.66 - 0.22 * wave(signT + 0.11);
    var signCCx = 0.50 + 0.22 * wave(signT * 0.50 + 0.73);
    var signCCy = 0.45 + 0.22 * wave(signT + 0.39);
    var signCCz = 0.55 + 0.20 * wave(signT * 0.75 + 0.17);

    var sdx = nx - signACx, sdy = ny - signACy, sdz = nz - signACz;
    var signA = clamp01(1.0 - (sdx * sdx + sdy * sdy * 0.82
                            + sdz * sdz * 0.58) / signRadiusSq);
    sdx = nx - signBCx; sdy = ny - signBCy; sdz = nz - signBCz;
    var signB = clamp01(1.0 - (sdx * sdx + sdy * sdy * 0.82
                            + sdz * sdz * 0.58) / signRadiusSq);
    sdx = nx - signCCx; sdy = ny - signCCy; sdz = nz - signCCz;
    var signC = clamp01(1.0 - (sdx * sdx + sdy * sdy * 0.82
                            + sdz * sdz * 0.58) / signRadiusSq);
    signA = pow(signA, signEdge);
    signB = pow(signB, signEdge);
    signC = pow(signC, signEdge);
    var signDye = clamp01(signA * 0.90 + signB * 0.75 + signC * 0.65);

    bri = 0.20 + base * 0.55 + flow * 0.16
        + ink * (0.06 + signDye * 0.58);
    blend = clamp01(0.12 + ink * (0.15 + signDye * 0.72));
  }
  ...
  // Vintage rails alone receive incandescent droplets. W and A are always
  // driven together; RGB gold supports the physical warm emitters.
  var ww = 0.0;
