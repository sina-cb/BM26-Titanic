...
export var auroraHeight = 0.6;
export var windShimmer = 0.25;
export var cabinWarmth = 0.3;
...
export function render3D(index, x, y, z) {
  ...
  var yPhase = (y * 2.0 - tPhase) % 1.0;
  if (yPhase < 0) yPhase += 1.0;
  var noise = wave(yPhase);
  var r = (pr1 + (pr2 - pr1) * noise) * (isRedwood ? auroraHeight : 0.1);
  var g = (pg1 + (pg2 - pg1) * noise) * (isRedwood ? auroraHeight : 0.1);
  var b = (pb1 + (pb2 - pb1) * noise) * (isRedwood ? auroraHeight : 0.1);
  var w = isRedwood ? (windShimmer * (random(1) < 0.05 ? 1.0 : 0.0)) : 0.0;
  var a = ((viewMask & 2) != 0) ? cabinWarmth : 0.0;
  var u = uvIntensity * (1.0 - noise);
