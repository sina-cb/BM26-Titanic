...
export var shadowDepth = 0.5;
export var abyssalSwell = 0.4;
export var edgeShimmer = 0.25;
...
export function render3D(index, x, y, z) {
  ...
  var shadowMask = wave(x * 2.0 + tPhase) > shadowDepth ? 1.0 : 0.05;
  var r = 0.0, g = 0.0, b = 0.0, u = 0.0;
  if (isApex) {
    var pulse = wave(y - tPhase * 2.0);
    r = (pr1 + (pr2 - pr1) * pulse) * abyssalSwell;
    g = (pg1 + (pg2 - pg1) * pulse) * abyssalSwell;
    b = (pb1 + (pb2 - pb1) * pulse) * abyssalSwell;
  } else {
    r = pr1 * 0.1 * shadowMask;
    g = pg1 * 0.1 * shadowMask;
    b = pb1 * 0.1 * shadowMask;
    u = shadowMask * uvIntensity;
  }
  var w = edgeShimmer * (random(1) < 0.01 ? 1.0 : 0.0);
  var a = 0.0;
