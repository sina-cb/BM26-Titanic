...
export var fireFlicker = 0.5;
export var boilerHeat = 0.5;
export var valvePressure = 0.3;
...
export function sliderFireFlicker(v) { fireFlicker = v; }
export function sliderBoilerHeat(v) { boilerHeat = v; }
export function sliderValvePressure(v) { valvePressure = v; }
...
export function render3D(index, x, y, z) {
  var flicker = wave(tPhase * fireFlicker * 10.0 + index * 0.1);
  var heat = boilerHeat * (0.8 + 0.2 * flicker);
  var r = heat;
  var g = heat * 0.25;
  var b = 0.0;
  var a = ((viewMask & 8) != 0) ? heat : 0.0;
  var isApex = (viewMask & 3) != 0;
  var w = isApex ? (valvePressure * (random(1) < 0.05 ? 1.0 : 0.0)) : 0.0;
  var u = uvIntensity;
