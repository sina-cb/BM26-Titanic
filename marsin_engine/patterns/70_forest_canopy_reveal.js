...
export function sliderCanopyReveal(v) { canopyReveal = v; }
export function sliderLanternGlow(v) { lanternGlow = v; }
export function sliderCanopySparkle(v) { canopySparkle = v; }
...
export function render3D(index, x, y, z) {
  ...
  var pulse = wave(tPhase);
  var u = isRedwood ? (canopyReveal * pulse) : 0.0;
  var a = isVintage ? (lanternGlow * wave(tPhase * 4.0 + index * 0.05)) : 0.0;
  var w = isRedwood ? (canopySparkle * (random(1) < 0.02 ? 1.0 : 0.0)) : 0.0;
  var r = pr1 * 0.1, g = pg1 * 0.1, b = pb1 * 0.1;
