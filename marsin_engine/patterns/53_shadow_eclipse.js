...
export var eclipseDepth = 0.65;
export var coronaBloom = 0.4;
export var rimShimmer = 1.0;
...
export function sliderEclipseDepth(v) { eclipseDepth = v; }
...
export function render3D(index, x, y, z) {
  var cx = 0.5 + 0.3 * cos(tPhase * PI2);
  var cy = 0.5 + 0.3 * sin(tPhase * PI2);
  var d = sqrt((x - cx) * (x - cx) + (y - cy) * (y - cy));
  var inShadow = d < eclipseDepth ? 0.1 : 1.0;
  var rim = abs(d - eclipseDepth) < 0.05 ? 1.0 : 0.0;
  var r = pr1 * inShadow;
  var g = pg1 * inShadow;
  var b = pb1 * inShadow;
  var w = rim * coronaBloom;
  var a = ((viewMask & 8) != 0) ? (coronaBloom * rim) : 0.0;
  var u = rim * rimShimmer;
