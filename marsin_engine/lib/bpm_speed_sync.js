/**
 * BpmSpeedSync — drive CPC `speed` from `tempoBpm` when enabled.
 *
 * See docs/25_marsin_audio_analysis.md §6 for the design.
 *
 * Behaviour:
 *   - Subscribes to ParamCenter via `paramCenter.subscribe(...)`.
 *   - On every change event that touches `tempoBpm`, if `bpmSpeedSync`
 ...
 *     (OSC `/lx/tempo/bpm` today, mic-derived BPM in the future).
...
export class BpmSpeedSync {
  ...
  _onChange(ev) {
    ...
    const bpm = Number(params.tempoBpm?.value);
