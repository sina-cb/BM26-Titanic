 ...
 *
 * Usage:
 *   node tools/osc_live_listener.mjs            # default port 10000
 *   node tools/osc_live_listener.mjs --port 6970
 *
...
function parseArgs() {
  ...
  for (let i = 2; i < process.argv.length; i++) {
    ...
    if (a === '--port')        port = parseInt(process.argv[++i], 10);
      ...
      console.log('Usage: node tools/osc_live_listener.mjs [--port N] [--host H] [--duration S]');
      process.exit(0);
    }
  }
