...
const tmpStateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'marsin-retention-states-'));
const playlistsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'marsin-retention-playlists-'));
...
let proc = null;
let port = 6810 + Math.floor(Math.random() * 60);
...
after(async () => {
  if (proc && !proc.killed) {
    proc.kill('SIGTERM'); await sleep(500); if (!proc.killed) proc.kill('SIGKILL');
  }
});
