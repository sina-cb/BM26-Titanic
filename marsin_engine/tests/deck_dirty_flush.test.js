...
const tmpStateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'marsin-flush-states-'));
const playlistsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'marsin-flush-playlists-'));

let proc = null;
let port = 6740 + Math.floor(Math.random() * 60);
const BASE = () => `http://127.0.0.1:${port}`;

function spawnEngine() {
  const child = spawn('node', ['engine.js', '--pattern', PAT_B, '--model', SCENE, '--port', String(port)], {
    cwd: engineDir,
    stdio: ['ignore', 'pipe', 'pipe'],
    env: { ...process.env, BM26_DISABLE_TIMELINE: '1', MARSIN_STATE_DIR: tmpStateRoot, MARSIN_PLAYLISTS_DIR: playlistsDir },
  });
...
before(async () => { proc = spawnEngine(); await waitForReady(BASE()); });
after(async () => {
  if (proc && !proc.killed) { proc.kill('SIGTERM'); await sleep(500); if (!proc.killed) proc.kill('SIGKILL'); }
});
