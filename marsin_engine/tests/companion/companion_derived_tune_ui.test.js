...
test('DERIVED TUNE is a fourth accessible navigation panel', () => {
  ...
  assert.match(APP, /const pages = \['design', 'mic', 'derived', 'osc'\]/);
...
test('noteTracking inputs offer only what the validator accepts', async () => {
  ...
  for (const [key, id] of Object.entries(inputs)) {
    ...
    assert.equal(attrOf('min'), spec.min, `${key} min matches the validator`);
    assert.equal(attrOf('max'), spec.max, `${key} max matches the validator`);
