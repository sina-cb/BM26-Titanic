...
for (const [label, body] of [
  ...
  test(`loadSceneAudio — THROWS on existing ${label} YAML root`, () => {
    const dir = tmpScene();
    fs.writeFileSync(sceneAudioPath(dir), body);
    assert.throws(
      () => loadSceneAudio(dir),
      /must contain a YAML object|empty, scalar, and array roots/,
    );
    assert.equal(fs.readFileSync(sceneAudioPath(dir), 'utf8'), body);
  });
}

test('saveSceneAudio rejects non-object roots before touching disk', () => {
  const dir = tmpScene();
  assert.throws(() => saveSceneAudio(dir, null), /non-array object/);
  assert.throws(() => saveSceneAudio(dir, []), /non-array object/);
  assert.equal(fs.existsSync(sceneAudioPath(dir)), false);
});
