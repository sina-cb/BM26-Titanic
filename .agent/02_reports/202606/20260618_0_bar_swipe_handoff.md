...
## Naming (resolved)
The operator confirmed **`par_swipe`** — it lights **ONLY the pars** (fixtureId 1..4) and swipes across them 0→1, direction by a param. Name the file `par_swipe.js`. (The git **branch** is `claude/bar-swipe` — created before the name was settled; keep working on it, the branch name is just a label.)
...
This branch (`claude/audio-corpus-tuning-olcd6i` → `claude/bar-swipe`) carries the whole audio + patterns effort. The most relevant reports to read for this work:
