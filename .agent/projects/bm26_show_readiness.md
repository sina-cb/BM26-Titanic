...
The current shared checkout is in a stabilization-and-blessing phase. Active
ownership is split among Live Touch and
non-overlapping Timeline files. `api_server.js` is serialized under Live Touch.
Baby content remains reserved for Claude.

| Workstream | Current state | Evidence / next gate | Owner |
| --- | --- | --- | --- |
| **Live Touch production stabilization** | **OFFLINE-GREEN / PHYSICAL SMOKE OPEN.** Pattern switching, instruments 128/129/130, single-brush Spatial, Color, 16 Performance slots, non-replacing overlays, TAKE, full 964/964 Spatial projection, and authoritative overlay cleanup/DISARM are validated. Cleanup no longer calls retired `/movement-rate`; focused cleanup 36/36 and independent engine/session 40/40 pass. | Restart the engine, reload CaptainPad, then run physical ARM, pattern, effect, TAKE, Spatial add/erase, and DISARM smoke. Report `_331`. | `[Sub-Agent] Live Touch Production Stabilization` — offline complete |
| **Audio configuration** | **CLOSED / PHYSICALLY ACCEPTED / ARCHIVED.** Native Fabric/Yoga gave `SignalColumn {flex:1}` no intrinsic height inside the auto-height wrapped meter grid, collapsing normal flow so the updating monitor remained visible while lower configuration cards disappeared. Both roots now use `width:'100%'`; web remains healthy. | Sina confirmed the corrected native iPad Audio page is GOOD. Audio/native 59/59, full CaptainPad 2510/2510, engine audio config/API 63/63, independent review, iOS Hermes export, and bundle probes pass. Full TypeScript has only two unrelated Deck-test diagnostics. Report `_333`. | `[Sub-Agent] Audio Configuration UI Debug` — archived |
| **Timeline hardening** | **COMPLETE; PHYSICAL-NATIVE SMOKE OPEN.** Exact armed-owner matrix 1/1, Timeline 450/450, CaptainPad 76/76, adjacent APIs 5/5, independent validation, and inspected AFTER captures pass. | Sina repeats the documented physical-iPad smoke when convenient. | `[Sub-Agent] Timeline Lease and Reliability Hardening` |
| **Timeline priority over Live Touch** | **AUTOMATED-GREEN / PHYSICAL SMOKE OPEN.** Exact Timeline mutations force-clear and confirm ARM/source-lock release, hold authority through response, then dispatch once; overview remains ownerless. API 1/1 (including paused-body re-arm refusal), adjacent lease/takeover/deadman 10/10, gate/classifier 10/10, CaptainPad 62/62, TypeScript, lint, and web export pass. | On physical iPad, arm Live Touch and verify Timeline save, activate, cue, travel, and Party Mode each show PREEMPTING then succeed once with no old-plan flash. Report `_317`. | `[Sub-Agent] Timeline Priority Over Live Touch` |
| **Mixer adaptive uniform channels** | **AUTOMATED-GREEN / VISUAL GATE OPEN.** Equal dynamic sizing is implemented with today's width as the minimum and 50% as the maximum. Focused 87/87, full CaptainPad 2555 pass/6 skip, lint, and web export pass; Mixer has zero TypeScript diagnostics. | Capture honest 2/3/overflow states after explicit isolated-scratch authorization or in a later physical smoke; the read-only live stack currently exposes zero Mixer channels. Report `_318`. | `[Sub-Agent] Mixer Adaptive Uniform Channels` |
| **Deck/Mixer palette library polish** | **AUTOMATED-GREEN / VISUAL SMOKE OPEN.** The common ColorsWindow/picker uses one bounded Fabric/Yoga library layout. Saved-palette deletion is confirmed and updates only from the acknowledged server list; starred curated items remain immutable. Crossfade new-run endpoints come from current A/B instead of a stopped stale ring, while active Crossfade remains engine-authoritative. | Focused palette/layout/wiring/crossfade 354/354, TypeScript, and touched ESLint pass. Run the Deck/Mixer manual iPad/web smoke for narrow layout, confirmed deletion, and latest-A/B Crossfade. Report `_319`. | `[Sub-Agent] Deck and Mixer Palette Library Polish` — automated-green |
| **Color-wheel five-slot consistency** | **DIAGNOSED ONLY.** Every scheme generates five semantic slots; duplicate hues overlap at identical SVG coordinates. No files or tests changed. | Add a pure stable-key marker layout with deterministic collision offsets/leader anchors; preserve raw hues, A/B ordering, and scheme math. | `[Sub-Agent] Color Wheel Five-Slot Consistency` — diagnosis complete |
| **Crisp playlist** | **OPERATOR-APPROVED / CLOSED / ARCHIVED.** Keepers 01/02/03/06/08/10 are in both 53-entry Ambient playlists for Titanic/test_bench; scene pairs are byte-identical. Crisp playlist/gallery retired; sources remain. Pattern 03 is bounded at Local Speed 0.20. | Automated, visual, audio, coverage, seam, TE-sign, lane, and performance gates pass. Sina approved the final Ambient gallery and closed the task. | `[Sub-Agent] Crisp Patterns Fresh Art Direction and Tuning` — archived |
| **Titanic model reference** | `docs/TITANIC_MODEL.md` and the narrow terminology correction in `docs/MARSIN_ENGINE_PATTERNS.md` are complete. LEFT=`X+`, RIGHT=`X-`, FRONT=forward end, BACK=rear end, UP=`Y+`. The legacy runtime naming inversion is explicit. | 24/24 census rows, 26/26 links, and focused coverage tests pass. Workstream closed; future production-family coverage is a separate follow-up. | `[Sub-Agent] Titanic Model Documentation` — closed/archived |
| **Baby Reveal patterns** | Codex work paused and archived to avoid shared-tree collisions. **Unified and rebuilt in `_306`** — one `baby_reveal` playlist, ten colour-blind patterns, `patterns/baby/` deleted. | Claude owns current Baby changes and operator review. | Claude + Sina |
| **Effects overhaul (plan phase)** | `_310` review complete: its audit/gallery and E1–E30 proposals are real, but zero overhaul product code exists. Direct Kick Punch and Hi-Hat Sparkle inputs are hard-coded to zero; shared freshness and musical phase remain incomplete. | Wait for Live Touch P0, obtain Sina's proposal rulings, repair audio/BPM authority, productize gallery generation, then implement a small gated first wave. | `[Sub-Agent] Effects System Review` — complete |
| **Deployment hardening** | Local implementation complete: secrets provisioning, `.git` exclusion/ACL repair, `--no-launch`, and verified desktop shortcuts/icons. No remote deploy was run by an agent. | Operator-controlled dry run/deploy when desired. | deployment sub-agent — complete |
| **Commit readiness** | Audit complete; nothing staged. Runtime state and machine-written bench mirror state are excluded from any future checkpoint. | Re-audit the final integrated diff and run the staged security gate only when Sina explicitly requests a commit. | coordinator |

Current operator gates are: physical-iPad Live Touch acceptance, Claude's Baby
handoff, and any operator-run exterior deploy. No agent
currently has permission to commit, push, arm the rig, or manipulate live
services.

Coordinator reporting rule: every five-minute update lists every tracked
`[Sub-Agent]`, including unchanged state, and an explicit `Needed from Sina:`
line. Material changes are compacted into this dossier, the live tracker, and
`context/now.md`; routine coordination remains agent-owned.
...
## Historical thread checkpoint — 2026-08-16
...
| **Live Touch production overhaul** — spatial-first, ambient backgrounds, deck colours, per-scene presets | **W1-W4 SHIPPED (`_288` shell+engine, `_289` panel). F2 CLOSED. Only F6 remains.** Design `_284` = `docs/70`. **Proof 1/14 -> 22/22** both docs/66 viewports. F2, the operator's central clutter complaint: portrait pad moved from starting **68% down (below the fold) to 26%**, legacy wheel **160,728px2 -> 0** (docked), landscape pad **81k -> 163k**. 44pt hit-region failures **40/81 -> 0/0**, still 0 after 22 controls were added (0/168), box census FLAT — `::after` overlays only, `_268` pad budget intact. After-shots `~/tmp/live_touch_impl/shots/after_289/`. **`_288`:** W1 shell (SPATIAL boots via `data-mode` — ordinal in FIVE places, not the two docs/70 named; POOL->INVERT label-only; residue deleted; F1 header fixed; DRAW/INK/TAKE real pill chrome; BRUSH cluster; audio rail `LOW`/`DOM1 FREQ`+Hz) + all three engine slices (W2 `{playlist,entryId}`+`applyEntryDefaults`; W3 the colour fan-out — armed Live literally could not see the daemon; W4 `live_touch_preset_manager.js`, atomic, autoSave-independent). **`_289`:** W2 picker 3 -> 37 options (BACKGROUNDS + INSTRUMENTS; `label:null` on all 34 so names are humanized; params never fetched); W3 three `data-color-card` daemon cards, client Scriabin DELETED (one note->colour authority), LEGACY COLOR docked via markup only (no version bump, no force-dock); W4 engine-backed preset playlist + D10 migration proven exactly-once. **THREE CONTRACT CORRECTIONS:** F1 is NOT in CaptainPad (no header chrome there) so W1 needed no rebuild; `PUT /layers/live_touch/pattern` has NO 409 precondition to preserve; all 34 `ambient.yaml` entries have `label: null`. **HAZARDS CAUGHT:** a ~10-min LIVE INVERSION (`docs/ui/` is served live to the iPad); the acceptance gate itself measured the wrong thing (`getBoundingClientRect()` cannot see `::after`, would have forced the exact `_268` regression) — now `union(box, ::after)`; and both colour panels initially shipped docked, leaving NO colour surface. **Gates:** engine contract 33/33; simulation panel 68/1 (pre-existing); CaptainPad 2336/0 + tsc 0; live_touch 21/21; state 128/128; colour-autopilot 122/122; ws_connect_replay 5/5; security PASS; all pins byte-identical. **ENGINE RESTART REQUIRED** (the `_288` slices; `_289` adds no engine code). **NO CaptainPad rebuild.** A panel reading "Presets store error" is CORRECT until that restart — fail-loud, not a bug. **ONE DEFERRED CHECK:** the live preset create->rename->reorder->delete->reload round-trip needs the restarted engine (~2 min); refused mid-arm because a bench-mirror was relaying real sACN. **NEXT: F6 only** — contracted as `docs/70` §10 by `_290`, implements as `_291` (PLAY/EDIT grammar; D17 curation is a separate operator-blessed data pass). **`_301` (2026-08-17, Fable): native ARM abort `pixel view is not verified` ROOT-CAUSED + fix contracted** — verification PASSES on the iPad; the abort is `initialSpatialPrepareBody`→`brushPatch`→`padWorldPerPx` throwing the same string when the docked (`display:none`) Spatial panel leaves `screenGlyphs` empty (draw() early-returns on the 0×0 canvas; per-device `bm26_touch_layout_v2` layout is why web passed). Proven offline 9/9 (`~/tmp/live_touch_arm_repro/repro.mjs`, full canArm()===true with hidden canvas). Fix contract + regression spec in report `_301`: split the conflated guard, canvas-independent `worldBrushRadii()` for ARM staging (same verification gates, no fallback, Spatial never auto-opened), stroke path untouched, no CaptainPad changes. **`_302` (2026-08-17, Opus-managed Sonnets): the four-fix wave SHIPPED.** **W1 was ALREADY LANDED by the concurrent Codex editor** — verified compliant against the `_301` §5 contract and built on, not undone; this wave added the §6 cases it lacked (`simulation/tests/touch_control_arm_brush_geometry.test.js`, 7 tests). **FINDING: `_301` §6.6's parity prediction was wrong** — the canonical ARM radius is **~4.9% SMALLER** than the screen-derived one at the operator's pad aspect, because `fraction x width x per` is scale-invariant only while the fit is width-limited and design is 900x520 against a ~1.97 pad. Exact parity DOES hold at the design viewport (1.000000 both axes) and that is what is pinned; chasing parity at arbitrary aspects would mean reading the pad box, i.e. re-coupling to the canvas. Deterministic, bounded, one-directional (ARM can never stage a brush WIDER than drawn), and the first stroke re-asserts screen-true size. **W2 spatial-paint 400 FIXED client-side only** — the raw WKWebView `pointerId` was going on the wire past the engine's `0x7fffffff` cap (engine validation correct, untouched); now a ten-slot pool (smallest free 0..9, allocated at both creation sites, released at all three removal sites, exhaustion throws, no fallback), `pointer.id` still the raw Map key. **W3 EFFECTS tiles NEEDED NO CODE CHANGE — pure downstream symptom of W1**: tiles are provisioned ONLY by the ARM chain (`collectEffectSlotBuildOperations` + `verifyPreparedSlots` readback), so while ARM aborted the engine kept a PREVIOUS layout (live slots 9-13 = Invert/UV Blast/Fogger/Long Trails/Cosmic Trails) and every tile rendered `fx-unwired`, 0 active; all 16 `FX_DEFAULT` pairs verified present in the LIVE library (51 pairs, 0 missing); 3 new pins so a tile can never silently no-op. **W4 COLOR HUB clipping FIXED, CSS only** — `_289` landed every `.ch-*` row's spacing unconditionally (portrait-sized), landscape 11" caps `.panel-body` at 367px real vs 375 needed (437 with the DRIVING strip), and `.panel{overflow:hidden}` made it silent unreachable clipping; one landscape-scoped block recovers 76px via the file's own `::after` hit-region recipe (44pt preserved), FIT not scroll per docs/70 §10.2. **Gates:** simulation 63/63, engine contract 52/52, CaptainPad 28/28, tsc 0, security 6 findings ALL pre-existing in gitignored `.scene_backups/` and zero in touched files; transport pins hold and `theme.js`/`pixel_views.js` byte-identical to baseline. **NO ENGINE RESTART NEEDED, NO CaptainPad rebuild** — everything landed is `docs/ui/` (hot-served) plus tests. **`_302` REPORT-NUMBER COLLISION:** the Codex editor also wrote a `_302` report; two files share the number, nothing renamed — operator call. **FOLLOW-UP:** `pushXY()`'s `Number.isInteger(e.pointerId) ? … : TAKE_POINTER_ID` silently aliases a non-integer real pointer to the playback entry and DROPS that touch (latent P0 fallback). **NEXT: F6/`_291` only.** | Opus lead (`_288`, `_289`, `_302`) → `_291` for F6; Fable `_301` diagnosis |
...
| **Baby Tease redesign** — kill the bilateral-split monoculture | **PHASE 1 DESIGN COMPLETE (`_299`, Fable) → PHASE 2 IMPLEMENTATION SHIPPED (`_300`, Opus lead + 3 sub-agents).** The audit proved the operator's complaint mechanically (12 of 15 tease patterns were one thresholded ship-frame plane + <25 % wiggle = the same left/right split at 50 ft, plus an ad-hoc pink-gain zoo); `docs/72_baby_tease_pattern_redesign.md` is the contract. `_300` landed all 13 keepers — 3 reworked in place (10 braided rivers, 12 cellular organism, 13 yin-yang, the last gaining dominant hooks + counter-colour eyes) and 10 new (`82`–`91`) — on ONE canonical authority block that is byte-identical across the set (the emit helpers hash the same in all 13 files; the per-pattern gain zoo is gone). Playlist rebuilt byte-identical across both scenes with `defaults` generated from each pattern's own declared slider order, `pattern_goals.json` + manifest regenerated, gallery + combined index regenerated and VISUALLY inspected (no entry reads as a half-and-half split any more). Per the operator's cleanup order, `patterns/baby/` is now exactly the referenced working set: **33 sources = 33 manifest ids** (13 tease + 10 boy + 10 girl). Two NEW offline gates shipped in `marsin_engine/tests/patterns/baby_tease_redesign_metrics.test.js` (L2 anti-bilateral, §9 perceived balance, authority-block pin) — 3/3; `playlist_gallery_tool` 13/13; simulation `pattern_manifest` 6/6; `baby_color_contract` 15/16. Worst L2 0.26 mean / 0.43 max vs limits 0.35/0.65 (the deleted patterns measured 0.73–0.84); perceived balance 0.95–1.10; closest distinctness 12.73 vs a floor of 1.5; whole set re-validated at real playlist defaults after finding the gates drive every slider at 0.5. **D1–D8 all taken as proposed except D2** — `PINK_TRIM` ships 0.97 not 0.90 because the §9 weights already carry the Helmholtz–Kohlrausch compensation and 0.90 double-counts it (measured fail at 0.893); `docs/72` §11 D2 now points at the report. **OPEN FOR SINA:** (1) the Boy/Girl families were cut 20 → 10 each by the concurrent Codex writer, NOT by Sina — self-consistent, so `_300` did not restore them, but it needs ratifying or reversing (recipe in report §4.1); (2) the single red is the `constellation_flow` twin pair failing the bench gate, where HEAD's version measurably passes — recommendation is to restore that pair (report §4.2). **ENGINE RESTART REQUIRED** — the live engine still holds the old pattern set.  **OPERATOR FIELD REVIEW → v2 POLISH SHIPPED (`_305`, Opus lead).** Sina reviewed the 13-keeper set on the LIVE RIG and loves it; three orders, all landed. **(1) REORDER + OWN DIRECTORY** — the keepers moved `patterns/baby/{10,12,13,82-91}` → **`patterns/baby_tease/{01-13}`**, renumbered in PLAYLIST order, a top-level SIBLING (not `baby/tease/`) because a qualified pattern id carries exactly one directory segment in `api_server.js` VALID_PATTERN_NAME, `playlist_manager.js` VALID_PATTERN and `pattern_manifest.cjs` alike. Entry ids, labels, notes and saved defaults all unchanged, so CaptainPad state and the `baby_reveal` show still resolve; playlists still byte-identical (4435 B). **(2) SPEED RETUNE onto the operator's declared reference (global SPEED 25, `sliderLocalSpeed` 0.30)** — and the naive factors were wrong, because **the engine's global knob is EXPONENTIAL** (`0.25 · 16^speed`, `engine.js` createRenderLoop) composed with the linear local curve, so the true equivalences are 9.978 / 7.849 / **14.446** against the assumed 9.6 / 8.4 / 11. Ten of thirteen retuned in their INTERNAL base rates (saved sliders untouched); three the operator called perfect were not touched; **`08_checker_tide` ×3.5 and `09_candy_helix` ×3.0 are flagged ESTIMATES** awaiting a rig eye. **`docs/72` D3 is WRONG — there is no CaptainPad clamp**, the legal range is g ∈ [0.25, **4.0**] and max product **8×**, so every §8 runaway ceiling was 2× optimistic (before this wave too); wraps re-verified safe at the true max. **(3) INK DROPS — the port bow was not dim, it was OFF.** `Left Front Wall` (90 px) rendered exact black in EVERY frame forever: the marbled-sea grating's two sines ran at nearly the same spatial rate in antiphase along that wall, so `emitBlack(); return;` fired before any drop math and **no spawn-site change could have reached it**. Fixed with a decorrelated 7-constant grating (solved against vein + sea balance + L2 simultaneously; the first attempt reintroduced the bilateral split at L2 0.634), four sites moved onto the measured pixel cloud, and **every drop made internally 50/50** (core inside a counter-coloured halo) — load-bearing, because with two sites per phase every family assignment either balanced the colours (L2 0.48) or satisfied L2 (balance 0.637/1.133), never both. Proof: that wall **0.0 → 93.0 mean peak/px**, ever-lit **0.000 → 0.778**, zero-light regions **1 → 0**, region spread **infinite → 1.81×**, ink-reachable pixels 55.8% → 100%. Gates: `baby_color_contract` 15/16 (the red is `_300`'s pre-existing `22_boy_constellation_flow` bench failure), metrics 3/3, `playlist_gallery_tool` 14/15 (the red is the concurrent Crisp writer's), `pattern_manifest` 6/6; authority block still byte-identical across all 13. **Three gate constants were widened, each justified by probing the PRE-RETUNE sources** — the retune is a measurement ACCELERANT, not a regression: untouched `07_braided_rivers` already read 0.597 authority and `04_rotating_yin_yang` reads 0.657 both before and after its ×1.15, so the old 0.69-1.45 five-sample band was never an envelope property; the handoff bound became `docs/72` L7 AS WRITTEN (65% in 0.5 s on a 41-frame grid = strictly more measurement). Gallery regenerated AT the show operating point (`--global-speed 0.4`, recorded in the manifest) and visually inspected at t = 2/10/18 s on all 13 — no half-and-half reads, no alias artefacts. Manifest diff audited against a session-start snapshot: **baby-only, zero crisp changes**; the combined index was deliberately NOT rebuilt because it would have demoted the concurrent writer's Crisp gallery. **OPEN FOR SINA:** saved `sliderLocalSpeed` defaults are still 0.40-0.47 while the retune targets 0.30 (a playlist-loaded entry runs 1.25× fast until the knob comes down — one number per entry, deliberately not changed); the two speed estimates; D3's true range; and `03_star_exchange` is still the dimmest entry. **ENGINE RESTART REQUIRED — the pending bounce now carries `_300` AND `_305`.** | Fable `_299` design → Opus `_300` implementation → Opus `_305` operator-feedback polish; operator ratification pending |
| **Baby Reveal unification** — one playlist, one colour, ten new looks | **SHIPPED (`_306`, Opus design lead + 3 Sonnet slices).** Contract: `docs/73_baby_reveal_unification.md`. The operator's three orders all landed. **(1) ONE LIST:** `baby_boy` + `baby_girl` merged into a single **`baby_reveal`** playlist (byte-identical both scenes); `patterns/baby/` **deleted** (20 sources), both old playlists retired, 60 stale `baby/` keys cleared from `pattern_goals.json`. The answer is a **colour** now, not a playlist. **(2) PALETTE CARRIER — no new engine tier:** the family rides the engine's existing CPC slots `colorPalette1`/`colorPalette2`, which already survive pattern rotation three ways (`captureDefaults`/`applyEntryDefaults` skip CPC names; `finalizeCpcValues` runs LAST on every switch). The ONLY engine change is the `globals` show-action validator learning HSV — a pure authoring gap, proven because the runner's own end-of-show restore already pushes HSV through `setGlobals`, and the *timeline's* validator already accepted it. ColorAutopilot is already force-disabled at ARM, so nothing competes. **P0 REFUSAL, load-bearing:** `colorPalette1` always holds *some* value so "unset" is undetectable — patterns check a **two-slot handshake** (same hue, slot 2 at exactly `DARK_K`) and render **every pixel black** otherwise. This is not theatre: `setGlobals` treats a `source_lock` refusal as arbitration and continues SILENTLY, so with Live Touch armed the palette write is dropped — under a single-slot design the reveal would then run in whatever colour was loaded, **including the other family's**. Black is recoverable; the wrong gender is not. The deck colour wheel pins `s:1,v:1` on both slots so it can never arm this family by accident. Verified 0/964 px lit across 7 wrong-handshake cases on both models. `colorTransitionMs: 0` pinned so the palette SNAPS. `DARK_K` is a deliberate two-place coupling (pattern + both show YAMLs); the gate asserts all twelve agree. **(3) TEN NEW PATTERNS** `patterns/baby_reveal/01…10`, numbered in playlist order, on ONE authority block byte-identical across the family (md5 `976276359e82fe633bfac5077c2bfbd2`). The retired 20 failed *mechanically*: they declared `COLOR_*_DARK`/`COLOR_*_LIGHT`, set both to the SAME triple and averaged them (two-tone collapsed to brightness-only mush), and used raw `localX/Y/Z` instead of the smokestack ship frame. **Twin fairness is now structural** — one set of files, so pink and blue cannot differ; every metric measures identical to the digit across both palettes. **THE OPERATOR'S DARK-SHADE SUGGESTION MEASURED AND CONFIRMED, and it beats the alternative:** swept `DARK_K ∈ {0.18…0.45}` on the real titanic model against a primary-plus-black control — two-tone is **perfectly bimodal, 0.0% of lit mass in the valley** at every K, while primary+black smears **27%** into the valley and lights only 20.4% of the rig vs 29.8%. (The tease can afford primary+black because *hue* contrast does the work; with one family the dark tone replaces it.) Shipped **`DARK_K = 0.28`** — dark tone at 47/255, middle of the 20-65 dim band, 4.79:1 below primary. **Gates:** new `baby_reveal_contract` **12/12**, new `baby_reveal_palette_dispatch` **6/6** (driven against the SHIPPED show YAML), `baby_color_contract` **14/14**, `show_schema` **39/39**, `playlist_gallery_tool` **16/16**, sim `pattern_manifest` **6/6**, full `tests/timeline/` **20 files 0 fail**; manifest regen audited **baby-only** with `crisp/` byte-untouched. **Two review catches:** the reworked `baby_color_contract` was silently **measuring black** (its shared animated/distinct tests render reveal patterns unarmed, so all ten read peak delta 0 and "near-duplicates 0.00") — fixed by arming in that file's `compilePattern`; and my own gate's 10-sample grid was **under-measuring**, passing `09_lighthouse_fans` at 25.4% where a dense probe read 24.6% — the same truncation failure `_305` §7.2(d) documented, grid now 31 samples/30 s. **Beyond scope because the deletion caused it:** `playa_default.yaml`'s two Baby cues fired the deleted playlists and would have failed at fire time — repointed to `baby_reveal` with the handshake in each step's `globals`, and the curated `palette: baby_pink/baby_blue` keys **REMOVED** (a curated palette is hue-only `s=1,v=1` and would have overwritten slot 2 and blacked out the reveal — a real trap, now commented); also repaired the pre-existing dangling `e_baby_tease_two_color_world_walk` cue. **CaptainPad: ZERO changes and none needed** — it contains no occurrence of `baby_girl`/`baby_boy`/`baby_tease` at all; the Special Events tab is a pure renderer and the reveal is engine+YAML data, so no rebuild/tsc/vitest, and the concurrent `_308` index/mixer wave was never at risk. The pink↔blue correction re-issues the palette for free (re-firing replays the whole action list) — asserted, not assumed. **GALLERY BLOCKED (tooling, not defect):** `playlist_gallery/generate.mjs` has no palette concept (builds `--set` from playlist defaults; CPC keys are never defaults) so it renders this family black, and teaching it `--palette` touches the concurrent Crisp writer's file — scratch both-palette contact sheets at `C:/Users/TITANI~1/tmp/reveal_wave/sheet_{pink,blue}.png` are the visual evidence; combined index also not rebuilt. `pattern_audio_harness.mjs --set` now accepts `name=H:S:V` so hsv controls can be driven offline at all. **OPEN FOR SINA (D1-D7):** keeper count 10; `FAMILY_TRIM` 1.00 (no pink trim — nothing to balance with one family); `DARK_K` 0.28; confirm black-on-refusal; arc order; direction sliders on 5 keepers; whether the `playa_default` Baby cues should stay repaired or be retired for the SPECIAL EVENTS tab. **Eye-check:** `08_comet_lullaby` (11.9% lit, sparsest by design), then `03_constellation_flow`/`04_bubble_chorus` (~25%); `09_lighthouse_fans` is the thinnest margin on TWO independent gates (two-tone 24.6-25.4% vs 25% floor; anti-bilateral max P 0.643 vs 0.65). **ENGINE/LAUNCHER BOUNCE REQUIRED — the pending bounce now carries `_300`+`_305`+`_306`; an old engine re-scanning the new show YAML will REFUSE it (HSV `globals` needs the new schema). NO CaptainPad rebuild.** **OPERATOR RULING → PALETTE CONTRACT v2 SHIPPED (`_311`, Opus lead + 3 Sonnet slices).** Sina opened a reveal pattern on the DECK under an ordinary palette and got a black ship — v1's two-slot handshake working exactly as specified, and the wrong behaviour. Ruling: the patterns **consume the global colour palette**. `colorPalette1` is the primary; the second tone is now **DERIVED in the pattern** as that triple x `DARK_K`; `colorPalette2` is no longer read *or exported*; the handshake blackout is gone, so these looks animate on the deck in whatever colour is live. The only refusal left is an **INVALID palette** (a component outside `[0,1]`) -> black. **`DARK_K` stays 0.28 and no retune was needed — that is arithmetic, not luck:** v1 never rendered slot 2's colour either (it was a *token*; `emitDark` already emitted `triple x DARK_K`), and scaling an RGB triple by a scalar IS 'same hue, same saturation, value x 0.28'. **The safety moved to the dispatch path:** `setGlobals` swallows a `source_lock` refusal, so with Live Touch armed the palette write is silently dropped — under v1 that failed safe to black, under v2 it would run the ceremony in the PREVIOUS colour, possibly the other family's. The runner now READS THE PALETTE BACK (`_assertGlobalsLanded`, reusing the existing `captureGlobals` dep; ParamCenter's canonical state holds the TARGET value, so a match proves ACCEPTANCE not a finished fade) and **refuses the run** with a named error that reaches the ErrorStrip on the SPECIAL EVENTS tab carrying the key, both values and the source_lock diagnosis; the ship stays in HOLD WHITE 10% and the fix is a re-tap. **Measured VM property everyone measuring patterns needs:** the VM installs its own defaults and IGNORES the declared `export var` — `(0,1,1)` for an hsvPicker and **0.5 for every slider** — proven by four declared palettes (including an impossible `-1,-1,-1` sentinel) all rendering byte-identical RED, so 'absent palette' is undetectable and any offline render that does not push saved slider values is measuring 0.5. **Operator field review absorbed in the same wave** (set verdict 'great'): internal base-rate retunes `01` x0.45 (ESTIMATE — 'toooooo fast', no number given), `03`/`05`/`06` x1.15, `07` x1.70, all **probe-verified** against pre/post frame-delta (0.485/1.140/1.181/1.154/1.689); `08_comet_lullaby` ('too little blue') got density not speed — radii 0.30/0.19 -> 0.52/0.34 and tail ghosts 3 -> 5, **12.0% -> 25.9% lit** on titanic with coverage still 100% ever-lit. Two docs/73 §4.2 ceilings are now touched at the LEGAL MAXIMUM (`06` 1.84 s and `07` 1.25 s vs the 2 s ownership ceiling; `07` 2.01% vs the 2% aliasing ceiling) — **recorded, not clamped**, because the factors are the operator's own measured taste. **P0 CANDIDATE FOUND: `04_bubble_chorus` is rendering a TASK #69 MISCOMPILE.** Two independent rewrites render byte-identically to each other and differ from the shipped file on **40.6% of emitted bytes**; the pop envelope is DEAD and the radii ride a corrupted value, which is why it measures 87.2% of titanic lit instead of its designed ~25%. Left as it renders and loudly commented, because correcting the shape alone FAILS the coverage law (9.5% lit / **11 permanently black regions**; 22.9% / 4 at rBase x1.8) — `_306`'s coverage audit passed *because* the miscompile inflated the radii. Its look will change the day the VM is fixed. **Gates:** `baby_reveal_contract` **13/13** (rebuilt: purity relative to the armed hue under pink/blue/green x both models, invalid-palette-only refusal plus the load-bearing converse, a **NEW deck-usability gate** proving `R === 0 && B === 0` under a green palette, G5 on the DERIVED tone), `baby_color_contract` **14/14**, `baby_reveal_palette_dispatch` **10/10** (5 new refusal tests), `show_schema` 39/39, `baby_reveal_sequence` 5/5, full `tests/timeline/` 449/449, full `tests/special_events/` 122/125 (3 known foreign wedding reds). 60-row battery across three palettes x both models: zero foreign pixels, zero W/A/U, every keeper lit, every metric identical to the digit across hues. **ENGINE/LAUNCHER BOUNCE REQUIRED — the pending bounce now carries `_300`+`_305`+`_306`+`_311`. No CaptainPad rebuild; no playlist file changed.** | Opus `_306` design + implementation (3 Sonnet slices) -> Opus `_311` operator-ruling palette contract v2 + field retune; operator review of the retune + D1-D7 pending |
...
- **2026-08-17 — `_314` LANDED: COLORS panel live-apply + turns-window plan
  (docs/75).** Fable design for the operator's two orders: (1) colour/method/
  selection changes must retune a RUNNING two-colour/turns/follow-note program
  (root cause is client policy — the engine's PATCH retune already exists and
  the colour gestures never use it); (2) PALETTE TURNS must rotate the selected
  pair as a two-colour window queue with BOTH channels fresh each turn (today
  the window steps one slot, so adjacent pairs share a colour turn-to-turn).
  Fix is client-only (no engine change/restart): a retarget rule (running
  family ⇒ sparse `PATCH {palettes}`) plus a stepped orbit (s=2 adjacent /
  s=1 spaced, crossfade byte-identical) with the recognizer generalized.
  Implementation reserved as `_315` (W1 logic → W2 window ∥ W3 mounts → W4
  Opus validation); serialize `_315` W2 against `_297` W4 on
  `colors_window.tsx`. Report:
  `.agent/reports/202608/20260817_314_colors_panel_live_apply_plan.md`.

- **2026-08-17 — `_333` LANDED: native Audio configuration body restored and
  physically accepted.** The route and live monitor were healthy; Fabric/Yoga
  collapsed the auto-height wrapped meter row because both `SignalColumn`
  branches used `flex:1`, leaving the lower cards outside reliable flow.
  `width:'100%'` restores intrinsic height; the exact regression pins 0 px
  before vs 106 px after. Sina confirmed the native iPad page is GOOD. Focused
  Audio/native 59/59, CaptainPad 2510/2510, engine audio config/API 63/63,
  touched lint, independent review, and iOS Hermes export pass. Full TypeScript
  remains red only on two unrelated Deck-test diagnostics. No git, services,
  or live output.

- **2026-08-17 — `_315` LANDED: COLORS panel live-apply + turns rotating
  window queue (docs/75).** Both operator orders closed, client-only — no
  engine change, no engine restart, live stack never bound. (1) While a
  crossfade or PALETTE TURNS ring is running, every colour gesture
  (wheel/dial drag, chip, saved pair, preset, scheme tap, A/B pick) is now a
  sparse `PATCH {palettes}` through the daemon's own front door instead of a
  refusal or a full POST — so a new contrast/split/colour retunes the ongoing
  program in place, with generation, cursor, phase and the in-flight fade all
  surviving. The TURNS restage and the latched wheel drag switched POST→PATCH
  (throttled, trailing flush on release), retiring a POST-storm that had been
  restarting the rotation under the operator's finger. FOLLOW NOTE and
  palette-set refusals unchanged. (2) The orbit gained a **step** — s = 2 for
  adjacent picks, s = 1 for spaced, crossfade byte-identical — so the
  selected two rotate as a real window queue with BOTH channels landing on
  fresh colours every turn, instead of channel A inheriting what B just
  showed. All D1..D7 taken at their defaults; **D2 deliberately retires the
  `_224` adjacent-slide look for everyone** (that is what the complaint was
  about) and **D4's instant-landing engine slice was skipped and stays
  deferred** (retargets land at the next transition — up to a turn late at
  HOLD 30/60 s). The TURNS card gained an arm-then-tap A/B pick (D5).
  Contract correction recorded: docs/75 §4's "try s = 1 first" recognizer
  order is wrong and would have silently renumbered the operator's T1..T5;
  `turnsOrbit` now disambiguates against the staged ring. docs/61 §5's
  crossfade stage-only row is marked superseded. Gates, all re-run by the
  lead: CaptainPad vitest 143/143 files / 2582 pass, **37 wiring pins green
  with that file byte-identical to HEAD**, tsc clean, expo lint 0 errors,
  security check 0 findings in `_315` files, engine `color_autopilot` suite
  105/105 (regression spec 1 proven offline). Two worker failures were caught
  in central review and repaired by the lead: an invented `livePalettes` wire
  field that would have thrown on every RUN CROSSFADE/START TURNS, and edits
  to the off-limits 37-pin file. **Rebuild the pad — YES.** Open flags:
  `CaptainPad/shared/` is untracked in git (no `git status` protection), and
  an out-of-scope `onDeletePair`/`opConfirm` change from `_297` territory
  rode in on `colors_window.tsx`. Report:
  `.agent/reports/202608/20260817_315_colors_panel_live_apply_impl.md`.
