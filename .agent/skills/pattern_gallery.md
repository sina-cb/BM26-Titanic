...
## Choose the gallery surface

There are two complementary gallery surfaces:

- **Permanent saved-playlist galleries** live in `docs/pattern_gallery/` and
  are generated by `marsin_engine/tools/playlist_gallery/generate.mjs`. Use
  these for a shareable gallery, complete playlist index, exact saved values,
  or committed ten-second top/front/feature GIFs.
- **Interactive local audition** remains `marsin_engine/tools/gallery/` and is
  the surface documented below. Use it for phone/Tailscale review, live deck
  control, comparison, or Static/Sound/model variants.

Generate a permanent gallery from `marsin_engine/`:
...
The permanent generator is offline and fail-loud. It uses the matching engine
model, exact playlist defaults, real harness, and repo-bundled FFmpeg binary;
it never boots an engine or binds a show port. Full procedure:
