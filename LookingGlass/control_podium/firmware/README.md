# `LookingGlass/control_podium/firmware/` — Heltec Radio Firmware
...
with `python LookingGlass/control_podium/firmware/deploy.py`). Direct `pio run …`
...
python deploy.py --node 0x0A            # captain handheld (name from nodes.yaml)
...
* **Requirements**: SSH access to the Pi configured in `LookingGlass/control_podium/server_bridge/.ssh.secret`.
* **Command (run from your laptop)**:
  ...
  PYTHONPATH=LookingGlass/control_podium python3 -m server_bridge.deploy --firmware
  ```
...
e.g. node 0x0A (`name: captain`) advertises as `tcon_captain`. A plain
