...
For the protocol spec and on-air frame design, see [`docs/22_server_bridge.md`](../../../docs/22_server_bridge.md).
...
cd ../..
PYTHONPATH=LookingGlass/control_podium python3 -m server_bridge.deploy

# 3. Verify bridge health and view remote logs
PYTHONPATH=LookingGlass/control_podium python3 -m server_bridge.deploy --verify-only
```
...
|--------------------|------------------------------|---------|-----------------------|
| **Pi SSH Credentials** | `LookingGlass/control_podium/server_bridge/.ssh.secret` | Defines the SSH target, credentials, and optional engine override for the deploy script. **GITIGNORED**. | **Yes**: Copy from [`.ssh.secret.example`](.ssh.secret.example) and fill in. |
...
| **Node Registry** | `LookingGlass/control_podium/.config.nodes.yaml` | Maps Node IDs (e.g., `0x01`, `0x0A`) to roles (`server`, `captain`) and USB MAC addresses. | **Yes**: Pair your Heltecs via `deploy.py` so MACs match the hardware. |
| **Command Allowlist** | `LookingGlass/control_podium/.config.commands.yaml` | Declares the permitted query/command endpoints and their minimum role requirements. | No (pre-configured for standard operations). |
| **Bridge Parameters** | `LookingGlass/control_podium/.config.bridge.yaml` | Configures the default engine URL, status broadcast intervals, and feature flags. | Optional (overridden by `.ssh.secret` `ENGINE_URL`). |
...
cp .ssh.secret.example .ssh.secret
```
Open `.ssh.secret` (copy from `.ssh.secret.example`) and specify:
* `HOST`: The bridge Pi on your LAN (e.g., `<BRIDGE_HOST>`).
* `USER`: The Pi username (set in `.ssh.secret`).
* `PASSWORD`: The sudo/SSH password (set in `.ssh.secret`).
* `INSTALL_ROOT`: Where the code will live (typically `/opt/titanic-bridge`).
* `ENGINE_URL`: MarsinEngine on the LAN (e.g., `http://<ENGINE_HOST>:6968`).
...
3. **Sync**: Syncs the `LookingGlass/control_podium/` directory, `marsin_engine/secret.yaml`, and docs to the Pi.
...
```bash
PYTHONPATH=LookingGlass/control_podium python3 -m server_bridge.deploy --firmware-only
```
...
```bash
curl -s http://<BRIDGE_HOST>:7099/health
```
Look for:
