...
INSTALL_ROOT. Optional: PORT (default 22), PASSWORD, ENGINE_URL.
  ...
  2. ``apt`` ensures ``python3-venv``, ``rsync``, and ``socat`` are
     installed on the Pi.
  3. ``rsync`` the bare minimum from ``LookingGlass/control_podium/`` and
...
CP_ROOT = HERE.parent           # .../LookingGlass/control_podium
...
def _step_rsync_repo(ssh: SSHRunner, install_root: str) -> None:
    ...
    # Push two trees: LookingGlass/control_podium/ (minus excludes), and
    ...
    # marsin_engine/secret.yaml lives outside LookingGlass/control_podium/. Ship
