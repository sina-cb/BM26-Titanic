...
PortWatch requires the shared AES-128 key from `marsin_engine/secret.yaml` to authenticate and encrypt LoRa communication.

### Development & Preview Builds (Local & Simulator)
In development, the key is baked directly into the JavaScript bundle.
1. Ensure your `marsin_engine/secret.yaml` is configured on your laptop (copy from `marsin_engine/secret.yaml.example`).
2. From the `LookingGlass/control_podium/PortWatch` directory, run:
   ```bash
   npm run sync-secret
   ```
   This generates the gitignored `src/_generated/secret.generated.ts`. The build will fail if this file is missing.
...
```
`prebuild` creates `LookingGlass/control_podium/PortWatch/ios/` (gitignored) and runs `pod install`. **First run takes 15-30 min** on a cold CocoaPods spec cache; subsequent runs are seconds. The `ios/` directory is regenerable from `app.json` + plugins, so feel free to delete and re-prebuild whenever native deps change.

### Step 2: Set your signing team
Use the Apple Developer **Team ID** (`<TEAM_ID>`) that owns `com.titanicrig.portwatch` and has your deployment iPads registered. Pass it as `DEVELOPMENT_TEAM` on the `xcodebuild` command line in Step 4 (don't bake it into `project.pbxproj`, since prebuild regenerates that file).

> Pick the team from **Xcode → Settings → Accounts** (Team ID column). Do not assume `security find-identity` will choose the right team — confirm the Team ID matches the account that owns the bundle id.

### Step 3: Find your deployment iPad UDID
```bash
...
| **`xcodebuild` exit code 65** / `ReactNativeDependencies` Script failed | Stale/invalid Node path in Xcode env cache. | Open `ios/.xcode.env.local` and update `NODE_BINARY` to a valid path (e.g. `export NODE_BINARY=/opt/homebrew/bin/node`). |
...
| **All Commands Time Out** | The server bridge on the Pi is down or cannot reach the engine. | Verify the bridge health: `curl http://<BRIDGE_HOST>:7099/health`. Ensure the Pi is pointing at your current engine host. |
