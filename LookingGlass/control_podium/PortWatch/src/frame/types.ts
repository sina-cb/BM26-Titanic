...
// Mirrors LookingGlass/control_podium/comms/frame.py exactly.
//
...
// Per .config.nodes.yaml `0x0B` is `captain_02` / type `ipad` /
// role `captain`. The captain Heltec the iPhone tunnels through is
// `0x0A` (`captain`); the bridge sees `src=0x0B` and applies captain ACL.
