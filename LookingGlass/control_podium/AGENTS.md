# AGENTS — Control Podium (archived under LookingGlass/)

If you are an agent picking this up, read this **first**. It's intentionally short.

> **Location:** `LookingGlass/control_podium/` (archived 2026-08). Python
> package name remains `control_podium`; from the repo root use
> `PYTHONPATH=LookingGlass` for `python -m control_podium.*` and pytest.
...
2. **Read `LookingGlass/control_podium/README.md`.** Top-level orientation and where-to-look-for-what.
...
5. **Run the test suite:**
   ...
   Run from the repo root with `PYTHONPATH=LookingGlass` (the test imports use
...
6. **Verify firmware still compiles** (don't skip this even on Python-only changes):
   ...
   ```bash
   cd LookingGlass/control_podium/firmware
...
* `python -m companions.mesh_demo` from the wrong directory or without
  ...
  `LookingGlass/control_podium/` and set `PYTHONPATH=.`.
* The pytest suite uses absolute imports — run it from the **repo root**
  with `PYTHONPATH=LookingGlass`, not from inside `LookingGlass/control_podium/`.
...
2. Full unit/sim suite passes (`PYTHONPATH=LookingGlass pytest LookingGlass/control_podium/tests/`).
