...
sys.path.insert(0, str(_PODIUM_DIR.parent))  # LookingGlass/, for `control_podium.*`
...
def podium_port(hil_pairings) -> str:
    ...
    ('captain') or role ('captain') from .config.nodes.yaml."""
    for key in ("captain",):
        if key in hil_pairings:
            return _port_for(key, hil_pairings)
    pytest.skip("HIL: no captain entry in .config.nodes.yaml")
