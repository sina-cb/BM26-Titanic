# `LookingGlass/control_podium/` — TITANIC LoRa Mesh + Bridge (archived)

> **Archived (2026-08):** This subsystem was moved from the repo-root
> `control_podium/` into `LookingGlass/control_podium/` as part of repo
> cleanup. It remains intact for reference, bench work, and Pi bridge
> deployment — but new operator-surface work belongs under
> `LookingGlass/panel_firmware/` and `CaptainPad/`. Active path:
> `LookingGlass/control_podium/` (Python package name stays
> `control_podium`; set `PYTHONPATH=LookingGlass` from the repo root).
...
2. **Heltec Radio Firmware (`firmware/`)**:
   ...
   * Reference: [`LookingGlass/control_podium/firmware/README.md`](firmware/README.md).
...
3. **Raspberry Pi Bridge (`server_bridge/`)**:
   ...
   * Reference: [`LookingGlass/control_podium/server_bridge/README.md`](server_bridge/README.md).
...
4. **PortWatch Field-Ops App (`PortWatch/`)**:
   ...
   * Reference: [`LookingGlass/control_podium/PortWatch/README.md`](PortWatch/README.md).
...
5. **Host Companions & Protocol Core (`companions/`, `comms/`)**:
   ...
   * Reference: [`LookingGlass/control_podium/companions/README.md`](companions/README.md) and [`LookingGlass/control_podium/comms/README.md`](comms/README.md).
...
│                                PYTHONPATH=LookingGlass pytest LookingGlass/control_podium/tests/ -q
│
...
```bash
PYTHONPATH=LookingGlass ../.venv-dev/bin/python -m pytest LookingGlass/control_podium/tests/ -q
```
...
```bash
PYTHONPATH=LookingGlass python -m control_podium.firmware.deploy --node 0x01 --build-only
PYTHONPATH=LookingGlass python -m control_podium.firmware.deploy --node 0x0A --build-only
# then with the boards plugged in:
PYTHONPATH=LookingGlass python -m control_podium.firmware.deploy --node 0x01
PYTHONPATH=LookingGlass python -m control_podium.firmware.deploy --node 0x0A
PYTHONPATH=LookingGlass python -m control_podium.companions.hil_companion_demo
```
