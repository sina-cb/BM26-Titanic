...
import { MidiDispatchContext, createDispatcher } from '../dispatch';
import { VIEW_MODE_CH, VIEW_MODE_CC } from '../vsn1_feedback';
...
describe('VSN1 small buttons (mode / view / bank / logo)', () => {
  it('sb_0 cycles the SELECTED slot mode (MODE button, never a view toggle)', async () => {
    const { manager, api, transport } = setup(drumSnap);
    await manager.start();
    ...
    // sb_0 is MODE, not a view change — it must emit NO view-mode CC.
  ...
  it('sb_2 is a no-op with banks SHELVED — nothing sent, nothing dispatched', async () => {
    // Multi-bank effects UX SHELVED 2026-07-14 (BANKS_UI_ENABLED=false): sb_2
    // early-returns and does NOTHING — it neither sends a MIDI frame nor
    // dispatches any slot/reset/bank action. drumSnap also carries no activeBankId,
    // so this doubles as the "refuse-unseeded" guard the shelved no-op subsumes:
    // whether or not a bank is seeded, an sb_2 press emits nothing. (The pre-shelf
    // seeded-cycle + anti-spurious-flip guards live in the SKIPPED block in
    // effects_v2 → scenarios/…; un-skip both when BANKS_UI_ENABLED flips back on.)
    const { manager, api, transport } = setup(drumSnap);
    await manager.start();
    transport.sent.length = 0;
...
// ── Behavior-aware slot dispatch on a NON-VSN1 device (the APC direct path) ───
// The full manager path proof for the Iceberg-Flash / White-Drop fix. A TRIGGER
// slot (a momentary drop-hit) must dispatch 'trigger', not the historically-
// hardcoded 'toggle' (a no-op flash-wise on a momentary effect). A TOGGLE slot
// still dispatches 'toggle', and a slot whose behavior hasn't reached the
// snapshot yet (a boot/refresh race) fails SAFE to 'toggle' rather than crashing.
// This is the NON-VSN1 (APC) counterpart to the DRUM-view slot-key contract
// above: the APC profile keeps the historical DIRECT single-press behavior (no
// select-first gate), so it complements — not duplicates — the VSN1 runtime.
// (Merged in from the retired global_effect_slot_behavior.test.ts.)

const apcSlotEndpoints: MidiEndpoint[] = [
  { id: 'in-0', name: 'APC mini mk2', portIndex: 0, kind: 'source' },
  { id: 'out-0', name: 'APC mini mk2', portIndex: 0, kind: 'destination' },
];

const apcSlotBaseSnap: MidiEngineSnapshot = {
  blackout: false, activePattern: null, patterns: [], globalEffects: {},
...
async function pressSlotPad(slots: MidiEngineSnapshot['globalEffectSlots']) {
  const snap: MidiEngineSnapshot = { ...apcSlotBaseSnap, globalEffectSlots: slots };
  const transport = new FakeTransport(apcSlotEndpoints);
  const api = makeApi();
  const manager = new MidiManager({
...
describe('behavior-aware global-effect slot dispatch (full manager path, non-VSN1)', () => {
  ...
  // dispatches immediately, no select-first gate. (The VSN1 runtime is proven in
  // the DRUM-view / selected-slot blocks above.)
