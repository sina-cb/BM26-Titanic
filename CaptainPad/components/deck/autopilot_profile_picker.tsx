 ...
 * owns the live `profile`/`profiles` state (reconciled off the `autopilot` WS
 * broadcast) and does the optimistic POST + rollback.
 */
 ...
 *  silent fallback: the operator sees exactly the id the engine sent). */
const PROFILE_META: Record<string, { label: string; hint: string }> = {
  random: { label: 'RANDOM', hint: 'Shuffle / sequential cycling (today)' },
  audio_reactive: { label: 'AUDIO REACTIVE', hint: 'Pick driven by live audio' },
};

function metaFor(id: string): { label: string; hint: string } {
  return PROFILE_META[id] ?? { label: id.toUpperCase(), hint: '' };
}

export function AutopilotProfilePicker({
  profile,
  profiles,
  onSelect,
  disabled,
}: {
  /** Active profile id (engine `playlist.autopilot.profile`). */
  profile: string;
  /** The set of selectable profile ids (engine `AUTOPILOT_PROFILES`). */
  profiles: string[];
  ...
  return (
    <>
      <TouchableOpacity
        ...
        accessibilityLabel={`Autopilot profile: ${currentMeta.label}`}
