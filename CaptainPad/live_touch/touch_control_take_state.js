...
(function installTouchTakeState(root) {
  ...
  function create(options) {
    ...
    function settleOutput(reason) {
      ...
      var wasPlaying = phase === 'playing' || phase === 'looping';
      generation += 1;
      clearTimer();
      phase = 'settling';
      publish();
      settlePromise = Promise.resolve(inFlight).catch(function () {
        ...
        var needsLift = contactMayBeDown
          || (wasPlaying && lastOutputPoint)
          || ((reason === 'clear' || reason === 'operator-stop') && !!lastOutputPoint && points.length);
        if (!needsLift) return null;
