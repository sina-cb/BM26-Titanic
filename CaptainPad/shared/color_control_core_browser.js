(function installColorControlCore(root) {
  'use strict';

  if (root.ColorControlCore !== undefined) {
    throw new Error('window.ColorControlCore is already installed');
  }

  const CORE_API_VERSION = 1;
  const COLOUR_EPS = 1e-6;
  const SCHEME_IDS = Object.freeze([
    'master', 'hue', 'complement', 'contrast',
    'analogous', 'triadic', 'split', 'tetrad', 'golden',
  ]);
  const SCHEME_BASE_S = 0.95;
  const MONO_STEPS = Object.freeze([1.0, 0.78, 0.58, 0.40, 0.25]);
  const COMP_OFFSETS = Object.freeze([0, 60, 30, -30, -60]);
  const SCHEME_MIN_V = 0.1;
  const SCHEME_ROTATION_MIN_V = 0.25;
  const ANALOGOUS_STEPS = Object.freeze([
    Object.freeze([0, 1]), Object.freeze([15, 1]), Object.freeze([-15, 1]),
    Object.freeze([30, 1]), Object.freeze([-30, 1]),
  ]);
  const TRIADIC_STEPS = Object.freeze([
    Object.freeze([0, 1]), Object.freeze([120, 1]), Object.freeze([240, 1]),
    Object.freeze([0, 0.55]), Object.freeze([120, 0.55]),
  ]);
  const SPLIT_STEPS = Object.freeze([
    Object.freeze([0, 1]), Object.freeze([150, 1]), Object.freeze([210, 1]),
    Object.freeze([150, 0.55]), Object.freeze([210, 0.55]),
  ]);
  const TETRAD_STEPS = Object.freeze([
  ...
  const MIN_CONTINUOUS_FADE_S = 0.1;
  const MIN_CONTINUOUS_METHOD_FADE_S = 0.1;

  function colour(h, s, v) {
    return { h, s, v };
  }

  function asHsv(channel) {
    return typeof channel === 'number'
      ? { h: channel, s: 1, v: 1 }
      : { h: channel.h, s: channel.s, v: channel.v };
  }

  function channelForWire(value) {
  ...
  function schemeFromSteps(steps, baseH) {
  ...
  function generateScheme(scheme, baseH) {
    switch (scheme) {
      case 'master':
  ...
  function orbitDistance(selection, ringLength) {
   ...
   * between the window's two ends: `d` says how far apart COLOUR A and COLOUR
   * B sit, `s` says how far the whole window jumps to reach the NEXT turn.
   *
   ...
   * We need to select two, and the window will move both in a rotating
   * window queue style").
   *
   * THE FIX: the smallest `s >= 1` for which two CONSECUTIVE windows never
   * share a slot — `{0, d} ∩ {s, s+d} = ∅ (mod n)` — so every turn hands the
   * rig two colours neither channel showed a moment ago. `gcd(s, n) === 1`
   * is required too, so a full lap of `n` turns still visits every slot
   * exactly once and wraps cleanly instead of orbiting a subset of the ring
   * forever.
   *
   * When NO such `s` exists — the crossfade's 2-slot ring can never produce
   * disjoint windows (there are only two slots to share), and the same is
   * true of any ring too short for a window to dodge itself — the search
   * returns `1`: today's behaviour, because there is nothing to fix. This is
   * what keeps the crossfade wire, and every ring shorter than 5,
   * byte-identical.
   */
  function orbitStep(distance, ringLength) {
    const n = ringLength;
  ...
  function orbitPairs(colours, selection) {
    if (colours.length < 2) {
    ...
    return colours.map((unused, index) => ({
  ...
  function turnsPairs(colours) {
    return orbitPairs(colours, [0, 1]);
  }

  function assertRotationTiming(holdS, fadeS) {
    if (!(holdS >= 0)) {
  ...
  function rotationAutopilotPatch(colours, holdS, fadeS, selection) {
  ...
  function turnsAutopilotPatch(colours, holdS, fadeS, selection) {
    return rotationAutopilotPatch(colours, holdS, fadeS, selection);
  }

  function crossfadeAutopilotPatch(hA, hB, holdS, fadeS) {
  ...
  function assertMethodTiming(holdS, fadeS) {
    if (!(holdS >= 0)) {
      throw new Error(
  ...
  function assertSchemeSubset(schemes) {
  ...
  function followNoteAutopilotPatch(args) {
  ...
  function paletteWritePayload(h1, h2) {
    return {
  ...
  function reduceColorControlState(previous, payload) {
    ...
    if (payload.followNote && typeof payload.followNote === 'object'
        && !Array.isArray(payload.followNote)) {
  ...
  root.ColorControlCore = Object.freeze({
    CORE_API_VERSION,
    COLOUR_EPS,
    SCHEME_IDS,
    SCHEME_BASE_S,
    ...
    COMP_OFFSETS,
    SCHEME_MIN_V,
    SCHEME_ROTATION_MIN_V,
    ANALOGOUS_STEPS,
    TRIADIC_STEPS,
    SPLIT_STEPS,
    TETRAD_STEPS,
    GOLDEN_ANGLE_DEG,
    MIN_CONTINUOUS_FADE_S,
    ...
    orbitPairs,
    turnsPairs,
    assertRotationTiming,
    rotationAutopilotPatch,
    turnsAutopilotPatch,
