...
**Step 4 — runtime (YOU).** Install **Node.js v24.16.0** (must match the
...
```
NOTE: `boot_server.ps1` is Phase 2 tooling and may not exist on the branch
yet. If it is missing, create the task anyway (it will fail harmlessly
until the file arrives via deploy) and flag it in your report.
