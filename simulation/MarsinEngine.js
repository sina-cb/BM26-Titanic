...
export class MarsinEngine {
  ...
  async init(wasmDir = './lib/marsin-engine') {
    try {
      ...
      // Emscripten CommonJS builds expose a global `MarsinEngineModule`.
      // Dynamic import() treats them as classic scripts in browsers, so
      // factory.default is undefined. Use a <script> tag instead.
      if (typeof window !== 'undefined' && typeof document !== 'undefined') {
        ...
        const factory = await import(moduleUrl);
  ...
  renderAll(pixelCount, coords = null) {
    ...
    result.set(this._module.HEAPU8.subarray(outPtr, outPtr + outSize));
