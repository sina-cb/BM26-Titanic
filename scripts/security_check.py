...
def snapshot(root: Path, files: list[str], include_unstaged: bool) -> Path:
    ...
    # Feed paths via stdin (`--stdin -z`) rather than as argv: a large commit
    # (e.g. a big merge) can exceed the OS command-line length limit — on
    # Windows this surfaces as CreateProcess WinError 206 and crashes the gate.
    out = _run(
        ["git", "checkout-index", f"--prefix={tmp.as_posix()}/", "-z", "--stdin"],
        cwd=root, input="\0".join(files),
    )
    if out.returncode != 0:
