...
`RESET ALL` atomically restores all 12 positions to this reference wheel. It is
guarded against an accidental podium tap: tap `RESET ALL` once to arm it, then
tap `CONFIRM RESET` within three seconds to apply the reset.
...
**Validated tier.** The estimator is gated at the `moderate` virtual-mic tier
(**18 dB SNR**) on a synthetic 124 BPM house progression. The checked-in gate
...
|---|---|
| harmonic palette | use `audioNoteHue`; it chooses one in-range partial, energy-gates weak input, rejects short flips, and holds color through gaps — inside its validated envelope only (18 dB SNR, sustained bass/root anchors, ~0.4–0.9 s to change; no chord or quality detection — see *What note tracking actually delivers*) |
