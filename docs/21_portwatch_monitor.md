...
> [!IMPORTANT]
> **Limited-Authority Principle:**
> PortWatch is a limited-authority field operations app. It is not a remote MarsinEngine shell. It exposes only the allowlisted commands defined in `.config.commands.yaml`. Fire/pyro, fixture patching, pattern source upload, arbitrary mixer writes, and direct DMX controls are structurally unavailable.
...
2. **Crypto correctness in a small blast radius.** Titanic Frame v2 uses
   AES-128-GCM with a 16-byte / 128-bit authentication tag, fingerprinted
   shared keys, and per-source sequence-number replay defence. The TS port
   has to be byte-exact compatible with `marsin_engine/comms/codec.py`. A
...
PortWatch (iPhone)
    │
    │  BLE (NimBLE, passkey-bonded, encrypted link-layer)
    ▼
...
PortWatch is the AEAD principal: it holds the AES-128 secret from `marsin_engine/secret.yaml` (baked in at build time) and signs every outbound frame. The payload written to `CHAR_CMD` is already a Titanic Frame v2 AEAD frame generated by PortWatch. The captain Heltec is a dumb radio relay — it does not know the AES key and cannot decrypt, forge, or modify frames. The bridge verifies every frame before it touches the engine. There is no unauthenticated path.
...
### 3.5 Counter Persistence Requirement

Every AEAD sender must maintain a monotonically increasing per-key, per-src counter. Nonce reuse under the same key is strictly forbidden. 

PortWatch must persist its next transmit counter in secure local storage (`expo-secure-store`), keyed by `portwatch.counter.${fingerprint}.${srcNodeIdHex}`. It must reserve a block of counters (e.g., blocks of 1024 values) to avoid writing to storage on every single packet, writing the next block limit to storage before using the counters in that block. This ensures that an app crash, restart, or reinstall cannot cause the counter to jump backward or reuse a nonce.
...
* **PYRO** (disabled) — The fire/pyro system is out-of-band. PortWatch must not carry fire commands, which are structurally unavailable.
...
* **SERVER BRIDGE** — green if a status pub/rep arrived in the last
  35 s; amber 35–60 s; red over 60 s. (Active publish cadence is 60s, but PortWatch polls every 8s as a backstop.)
...
* **PYRO CONTROL** (disabled), **HORN CONTROL** (status not wired) —
  out-of-band placeholders to clarify they are unreachable via this app.
...
6. Translate the validated command into a MarsinEngine REST call, wait
   ...
   the radio.
7. On a 60 s active / 120 s idle cadence, publish an engine status summary
...
| `sp`, `dr`, `ct`, `sz`, `rt` | CPC scalars (`speed`, `direction`, `count`, `size`, `rotate`) | Full CPC scalar set, compact-float encoded. PortWatch lifts these off the same PUB and merges into `globalParams` so CaptainPad nudges surface within ~PUB cadence (not the 8 s poll cadence). Omitted (not nulled) when the engine doesn't have the param registered. |
| `p1`, `p2` | CPC `colorPalette1` / `colorPalette2` | HSV triple as `<h>-<s>-<v>` (compact floats). Same propagation path as the scalars. |

> [!NOTE]
> **AsyncStorage Caching & Removal of plh/pph:**
> PortWatch currently uses a name-keyed AsyncStorage cache for playlist and pattern data. The older CRC32 compact-status hash fields `plh` and `pph` have been removed from the bridge and parser.
...
### 10.10 Playlist library and Pattern caching (AsyncStorage)

PortWatch implements a persistent local cache for playlists and pattern catalogues to minimise LoRa airtime. Because fetching lists over LoRa requires paginated requests (which can take several seconds to complete), caching is critical for a smooth user experience.

- **Cache layer:** Data is persisted locally using name-keyed `AsyncStorage` (see `src/state/persistedCache.ts`).
- **Hydration:** On app boot, the cache is hydrated from `AsyncStorage`.
- **Manual REFRESH:** Tapping the refresh button bypasses the cache, forcing a direct paginated LoRa query to the bridge.

The older CRC32 compact-status hash fields `plh` (playlist library hash) and `pph` (active playlist patterns hash) have been completely removed from the bridge, the parser, and the PortWatch application. Instead of checking hash changes on every status message, PortWatch relies on the durable name-keyed AsyncStorage cache, ensuring data stability and offline capability across app launches.

### 10.11 Sync architecture: dual-path (PUB broadcast + 8 s polling)

**The lesson:** broadcast alone is not enough.

The bridge pushes `compact_status` PUBs to PortWatch on a periodic + event-driven cadence (see `bridge.py::_status_publisher` and `_engine_ws_subscriber`). On a wired network that's enough. On BLE + LoRa, single-frame drops are routine:

* iOS can silently swallow a BLE characteristic notification under load (the app's monitor callback simply doesn't fire — no error).
* The half-duplex LoRa radio drops PUBs during contention (operator hammering, CaptainPad streaming params, pattern paging).
* One dropped PUB leaves `engineStatus` stale until the next event.

**Sync model: PUB broadcast + 8 s polling backstop.**

PortWatch unicasts three queries on a fixed cadence (status / globals default 8 s, local exports default 10 s, all tunable via `.config.portwatch.yaml::polling.*`):

| Hook                          | Query                  | Refreshes                                                                 |
| ----------------------------- | ---------------------- | ------------------------------------------------------------------------- |
| `useStatusPoller`             | `qry engine/status`    | The entire compact_status snapshot: `pat`, `pl`, `vw`, `vov`, `lk`, `lku`, `ap`, `apd`, `aps`, `sp`, `dr`, `ct`, `sz`, `rt`, `p1`, `p2`, `br`, `blk`, … |
...
| `useLocalExportsPoller`       | `qry exports/p/<n>`    | Per-pattern WASM sliders. Catches the case where CaptainPad nudges a slider WITHOUT changing the active pattern. |

Both pollers:
* Fire the first poll **immediately** on `connectGeneration` bump.
* Maintain a single in-flight guard — overlapping queries are skipped, not stacked.
* Are best-effort — failures fall through silently and the next tick tries again.
* Pause while disconnected; restart cleanly on every successful pair.

**How the two paths interact:**
* **PUB (event-driven, broadcast)** — *fast path*. Engine WS event → bridge subscriber wakes publisher → broadcast PUB arrives in ~hundreds of ms when nothing drops. Eager PUB is also sent on every HLO ACK.
* **POLL (cadence-driven, unicast)** — *reliable path*. Even when every PUB in a window drops, the next poll's REP rebuilds `engineStatus` end-to-end.

The bridge's status publish interval is **60 s when active and 120 s when idle** (configured in `.config.bridge.yaml::status_publish`). With 8 s polling as the reliable backstop, the periodic broadcast loop serves primarily as a safety check against a wedged WS subscriber.

### 10.12 Profile Picker UI Gating

The profile picker in `StatusScreen` (which allows switching named LoRa profiles) is a development and bench-only diagnostic tool. 

In production builds, the profile picker must be hidden or disabled unless the profile switching path is explicitly enabled and authenticated. Unauthenticated `*CFG` profile changes over LoRa are disabled in production configurations to prevent Denial of Service (DoS) attacks. Local USB recovery remains the primary recovery path if a node becomes orphaned.
...
sub-second reflection of CaptainPad changes, with the 8 s
...
### 13.6 PortWatch HIL & Production Readiness Checklist

This checklist details the hardware-in-the-loop (HIL) operational verification procedures required specifically for the PortWatch client app and its associated links:

* **Direct Link RF Performance**
  - [ ] **10-Meter Link**: Verify `100/100` successful pings (`pin` / `pon` round-trips) at an open distance of 10 meters.
  - [ ] **50-Meter Link**: Verify `100/100` successful pings at an open distance of 50 meters.
  - [ ] **100-Meter Link**: Verify `95/100` successful pings at an open distance of 100 meters.
  - [ ] **Obstruction Link**: Verify `95/100` successful pings when the direct line of sight is blocked by one realistic field obstacle (e.g., vehicle, wooden wall, metal box).
* **Recovery Gestures**
  - [ ] **PortWatch App Force Quit**: Force close the iOS app during an active connection. Reopen the app; it must re-establish the BLE connection and synchronize engine state within 10 seconds.
  - [ ] **Phone BLE Disconnect**: Turn off Bluetooth on the phone. Turn it back on; PortWatch must cleanly reconnect to the Heltec and verify that the next command counter is not reset or reused (prevents replay lockout).
  - [ ] **Client Heltec Reboot**: Reboot the captain Heltec. PortWatch must reconnect and resume transmitting frames using a counter strictly higher than before the reboot (verifying keychain/SecureStore durable persistence).
* **UI Sync & State Correctness**
  - [ ] **Cold Start Sync**: Open PortWatch before the engine state is known. Actionable controls (switches, sliders) must remain locked/disabled until a valid status response hydrates the state.
  - [ ] **Mixer Mode UI Safety**: Set the engine to mixer override mode. PortWatch must display the `TAKE OVERRIDE` button and hide active deck controls, preventing concurrent control conflicts.
  - [ ] **Control Lock Lease Expiry**: Request control lock from PortWatch. Stop PortWatch renewals (e.g., put the phone to sleep). Verify that the engine expires the lock after 30 seconds.
  - [ ] **CaptainPad Lock Overlay**: When PortWatch holds the control lock, verify that the CaptainPad interface displays the lock overlay.
  - [ ] **Optimistic Intent Drops**: Perform a toggle. If the engine rejects or disagrees with the intent post-ACK, PortWatch must drop the optimistic state and revert the toggle to match the engine.
  - [ ] **Out-of-Band State Sync**: Change parameters using CaptainPad. Verify that the changes propagate to PortWatch within 8 seconds (via poll) or immediately (via WS-triggered PUB).
* **Battery & Power Conservation**
  - [ ] **Low Battery Warning**: Reduce cell voltage to `3.40V`. Verify the OLED displays the low-battery warning banner.
  - [ ] **Critical Deep Sleep**: Reduce cell voltage to `3.10V`. Verify the Heltec enters deep sleep to protect the battery.
  - [ ] **BLE Keep-Awake**: Verify that the client Heltec does not enter deep sleep or drop power while a BLE session is active.
  - [ ] **Active Power Ramping**: Set `pin_high_forever` to `false` in `.config.firmware.yaml`. Verify that client Heltecs ramp down to LOW power (14 dBm LoRa, lower BLE TX) after 60 seconds of inactivity.
