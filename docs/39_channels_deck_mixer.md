# Design 39: Channels — Deck and Mixer
...
the channels-improvement campaign — production/operational
...
- **Loading a playlist / tapping an entry does NOT optimistically flip local
  state**: the deck transition takes ~Ns and the engine keeps reporting the
  prior entry until it completes. The panel arms a **pending-gate** from the
  response's `targetEntryId` and lets the WS broadcast (`deck` / `mixer` /
  `channelPlaylistData`) reconcile the final entry — so the row highlight
  settles on the loaded entry without bouncing. A watchdog (~8s) clears the
  gate if no broadcast arrives.
...
Entry-tap surfaces real failures via `Alert` (Codex P0 — no silent
...
- Arm the pending-gate from `targetEntryId` on deck entry loads.
...
- Optimistically flip the active entry on a deck entry load (it's still the old
  entry until `onComplete`).
...
| `api_server.js` `loadPlaylistEntryWithTransition` (~1278) | Deck soft-swap helper (used by entry advance) |
...
| `api_server.js` `/mixer/channels/:id/playlist/entry` (~4415) | Mixer instant entry advance |
...
- `POST /deck/overlays/:id/playlist` + `/playlist/entry` — load a playlist /
  load an entry.
...
- **`docs/19_playlists.md` §8.3 / §3.2 mark mixer-channel playlist routes as
  ...
  `POST /mixer/channels/:id/playlist/entry`, plus `/capture` and `/discard`.
  `19_playlists.md` was not edited (out of this doc's additive scope); this is a
  documentation lag to reconcile, not a code bug.
- **`docs/18_marsin_mixer.md` §7 lists `POST /mixer/base` and
  ...
  playlist-entry routes documented here; treat `18`'s §7 as the original
