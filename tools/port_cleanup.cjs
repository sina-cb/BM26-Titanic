...
function listenersOnPort(port, opts = {}) {
  ...
  if (IS_WIN) {
    const out = execFileSync('netstat', ['-ano', '-p', udp ? 'udp' : 'tcp'], { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
    const pids = new Set();
  ...
  try {
    const out = execFileSync('lsof', args, { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
